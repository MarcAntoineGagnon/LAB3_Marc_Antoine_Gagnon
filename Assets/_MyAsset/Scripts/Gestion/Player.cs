...
public class Player : MonoBehaviour
{
    ...
    private void MouvementsPlayer()
    {
        ...
        _direction = new(positionX, 0f, positionZ);

        // Sprint Lorsque le joueur appui sur shift
        if (Input.GetKey(KeyCode.LeftShift))
            _rb.velocity = _direction.normalized * Time.fixedDeltaTime * sprint; // Mouvement sprint
        else
            _rb.velocity = _direction.normalized * Time.fixedDeltaTime * _speed; // Mouvement normale

        // Rotation Joueur
        if(_direction.magnitude >= 0.1f) // Si le joueur est en mouvement uniquement (garde son angle lorsque immobile)
        {
